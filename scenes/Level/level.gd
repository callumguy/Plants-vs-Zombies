...
var RewardScene: PackedScene = preload(ScenePaths.REWARD)

const REWARD_MENU := preload(ScenePaths.REWARD_MENU)
const MAIN_MENU := preload(ScenePaths.MAIN_MENU) 
...
func player_wins(position: Vector2) -> void: # position of the last zombie (that just died)
    ...
    var lane = int(GridManager.pos_to_grid_pos(position).y)
...
func reward_collected() -> void:
    get_tree().change_scene_to_packed(REWARD_MENU)
    # self.queue_free()
    
func game_over():
    get_tree().change_scene_to_packed(MAIN_MENU) # back to menu
    # self.queue_free()
