...
func create_packets() -> void:
    ...
    plants_unlocked.append("jalapeno")
    plants_unlocked.append("tallnut")
    plants_unlocked.append("repeater")
    for plant in plants_unlocked:
        var packet = SEED_PACKET.instantiate()
        packet.set_script(load("res://scenes/Plants/SeedPackets/pickme_packet.gd"))
        print(packet)
